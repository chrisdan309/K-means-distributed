...
public:
    ...
    void run(vector<Point> &all_points){
        ...
        cout << "Clusters inicualizados = " << clusters.size() << endl
             << endl;
        ...
        while (true)
        {
            cout << "Iteracion N° " << iter << "/" << iters << endl;
            bool done = true;
            ...
            if (done || iter >= iters)
            {
                cout << "Clustering finalizado en iteracion : " << iter << "\n";
                break;
            }
            iter++;
        }
        ...
        outfile.open(filesystem::current_path().string() + "out.txt");
        if (outfile.is_open()){
            for (int i = 0; i < K; i++){
